...
\subsection{Video}
Per la cattura video esistono tre metodi di cattura:

\begin{enumerate}
	\item hook sulle librerie multimediali: tramite code injection su una libreria multimediale (DirectX, OpenGL, SDL, ecc\dots) si modifica il normale funzionamento di una funzione. Solitamente la funzione che invia il framebuffer della scheda video all'uscita;
	\item utilizzo delle API di framebuffer virtuale (ad es. X virtual framebuffer su Linux, Desktop Duplication API su Windows e Quartz Display Services su macOS);
	\item schede video appositamente progettate per lo streaming. La prima società a modificare una scheda video fu Sony che incluse un chip di codifica per il suo servizio di cloud gaming su GPU Nvidia ed AMD (rispettivamente PS3 e PS4) \parencite{PlayStation_Now_Chip}. Altre piattaforme che usano schede video con funzionalità di encoding incluse sono GeForce Now su schede Nvidia \parencite{GeForce_Now}, XBox Cloud Gaming su schede AMD \parencite{xCloudBlade}, Stadia su AMD \parencite{Google_Stadia_Server} e Luna su Nvidia \parencite{Amazon_Luna_GPU}. La lista delle aziende che vendono schede video con questa funzionalità è riportata in Tab. \ref{table:VideoCardEncodingFeature} \parencite{NVIDIAVideoCodecSDK} \parencite{GCNArchitecture} \parencite{IntelQuickSyncVideo} \parencite{Hexagon_DSP_SDK}.
\end{enumerate}

...
Al termine di ogni fase di incapsulamento la libreria \verb|AV Format| invia alla funzione di callback \verb|encode_to_movie::write_packet| il pacchetto (\verb|buf| di lunghezza \verb|buf_size|) appena codificato, che lo scrive sulla \verb|socket|. 
...
\begin{lstlisting}[caption=Codice per la codifica video, label={lst:videoCodecFFmpeg}]
	void add_frame(uint8_t[] pixels)
	{
		...
		sws_scale(
			encoder_context->video_converter_context, // contesto di conversione
			rgb_frame->data, // input
			rgb_frame->linesize, 0, height, // dimesione input
			yuv_frame->data, // destinazione
			yuv_frame->linesize); // dimensione destinazione		
...
\begin{lstlisting}[caption=Codice per la codifica audio, label={lst:audioCodecFFmpeg}]
	void add_instant(uint8_t[] stream, int stream_size, int stream_num_samples)
	{
		// Riempimento frame con audio da SDL
		avcodec_fill_audio_frame(
			wav_frame, // destinazione
			2, // canali input
		...
		// conversione audio
		swr_convert(
			encoder_context->audio_converter_context, // contesto conversione
			&convertedData, // destinazione
			output_frame->nb_samples, // campioni destinazione		
			wav_frame->data, // input
			wav_frame->nb_samples); // campioni input

		// riempimento frame destinazione
		avcodec_fill_audio_frame(
			output_frame, // destinazione
			encoder_context->audio_codec_context->channels, // canali destinazione
			encoder_context->audio_codec_context->sample_fmt, // formato destinazione
			convertedData, // input

...
I blocchi degli \textit{I-frame} sono trasformati dal dominio spaziale a quello delle frequenze utilizzando la trasformata discreta del coseno (DCT), definita in Eq. \ref{eq:DCT}, eseguita su $8\times8$ px \parencite{HyesookLim2000Asaf}:
...
Successivamente la matrice di 64 frequenze del blocco DCT viene quantizzata utilizzando la matrice di quantizzazione in Eq. \ref{matr:MPEG_Quant_Matrix}. La quantizzazione spazialmente adattiva è resa possibile dal fattore "scala di quantizzazione". Questo parametro può variare per ogni macroblocco all'interno di un'immagine per regolare in modo adattivo la quantizzazione.
...
La scansione zig-zag rappresente in successione in ordine crescente i coefficienti DCT.

\subsubsection{La codifica di Huffman}
I coefficienti AC sono codificati senza perdite secondo il \textit{run-length encoding} e la codifica di Huffman. Vengono formate tabelle (per Y, Cb e Cr) su base statistica. I coefficienti DC codificano le differenze tra blocchi dei macroblocchi.

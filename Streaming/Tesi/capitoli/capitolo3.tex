...
\subsection{Video}
\textit{MPEG-1 Video} supporta risoluzioni fino a $4095\times4095$ px ed un bit-rate fino a 100 Mbit/s. In Fig. \ref{fig:mpegPhases} è mostrato il processo completo di codifica video.
...
Ogni frame contiene macroblocchi, che sono l'unità d'elaborazione della compressione. I macroblocchi sono formati da $16\times16$ px e sono necessari ai fini del calcolo dei vettori di movimento e dei blocchi di errore per la compensazione del movimento. Essi si dividono in:
...
\begin{itemize}
	...
	\item \textit{B-frame} pul contenere tutti i tipi di macroblocchi (\textit{I}, \textit{P} o \textit{B}).
\end{itemize}

Ogni frame è scomposto in macroblocchi di $16\times16$ px a loro volta formati da 4 blocchi per la luminanza (Y di YCbCr) e 2 per la crominanza (Cb e Cr di YCbCr). Per ogni \textit{I-frame} i macroblocchi vengono ri-calcolati. Per i \textit{P-frame} e \textit{B-frame} i macroblocchi possono essere: ri-calcolati, copiati, spostati o modificati \parencite{MPEG_1_Video}.
...
%I macroblocchi sono organizzati in \textit{slice} divise nei 3 tipi: Y, Cb e Cr.

I layer di cui è composta una sequenza video MPEG-1 sono mostrati in Fig. \ref{fig:MPEG_layers}.
...
I blocchi degli \textit{I-frame} sono trasformati dal dominio spaziale a quello delle frequenze utilizzando la DCT, definita in Eq. \ref{eq:DCT}, eseguita sui blocchi $8\times8$ px della luminanza e della crominanza \parencite{HyesookLim2000Asaf}:
...
Successivamente la matrice di 64 frequenze del blocco DCT viene uniformemente quantizzata (operazione di divisione) utilizzando la matrice di quantizzazione in Eq. \ref{matr:MPEG_Quant_Matrix}. Questo processo introduce perdita di qualità del segnale.
...
Dopo la quantizzazione, il coefficiente DCT più basso (coefficiente DC) viene trattato in modo diverso dai coefficienti rimanenti (coefficienti AC). Il coefficiente DC corrisponde all'intensità media del blocco. I valori del quantizzatore diversi da zero dei restanti coefficienti DCT e le loro posizioni vengono quindi scansionati "a zig-zag" e codificati con l'entropia della lunghezza di esecuzione utilizzando tabelle di "codici a lunghezza variabile".
...
I coefficienti AC sono codificati senza perdite secondo il RLE e la codifica di Huffman, mentre i coefficienti DC codificano le differenze tra blocchi dei macroblocchi.

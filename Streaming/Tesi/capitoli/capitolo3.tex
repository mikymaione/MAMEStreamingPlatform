...
\subsection{Video}
Come detto precedentemente la maggior parte dei codec usano il modello di colori YCbCr, tra cui anche \textit{MPEG-1 Video}. Questo codec supporta risoluzioni fino a $4095\times4095px$ ed un bit-rate fino a 100 Mbit/s.

\textit{MPEG-1 Video} compone il filmato video di "gruppi di immagini" (GOP), ogni \textit{GOP} è composto da un solo \textit{I-frame}, da una sequenza di \textit{P-frame} e \textit{B-frame} e da un solo \textit{D-frame}, come mostrato in \ref{fig:I_P_and_B_frames}. Ogni GOP è formato da quattro diversi tipi di frame \parencite{VideoAndMultimediaTransmissionsOverCellularNetworks}:
...
\begin{itemize}
    ...
    \item \textit{P-frame} (\textit{Predicted-frame}) sfuttano la ridondanza temporale per aumentare la compressione. Memorizzano solamente le differenze nell'immagine rispetto al frame precedente;
...
Ogni frame è scomposto in macro-blocchi di $16\times16px$ a loro volta formati da 4 blocchi per la luminanza (Y di YCbCr) e 2 per la crominanza (Cb e Cr di YCbCr). Per ogni \textit{I-frame} i macro-blocchi vengono ri-calcolati. Per i \textit{P-frame} e \textit{B-frame} i macro-blocchi possono essere: ri-calcolati, copiati, spostati o modificati \parencite{MPEG_1_Video}.

\begin{figure}[H]
	\includegraphics[width=10cm]{immagini/macroblocchi}
	\centering

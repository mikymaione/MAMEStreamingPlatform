...
I blocchi degli \textit{I-frame} sono trasformati dal dominio spaziale a quello delle frequenze utilizzando la DCT, definita in Eq. \ref{eq:DCT}, eseguita su $8\times8$ px \parencite{HyesookLim2000Asaf}:
...
Successivamente la matrice di 64 frequenze del blocco DCT viene quantizzata (operazione di divisione) utilizzando la matrice di quantizzazione in Eq. \ref{matr:MPEG_Quant_Matrix}. Questo processo introduce perdita di qualità del segnale.
...
La scansione zig-zag rappresenta in successione (in ordine crescente) i coefficienti DCT.



\subsubsection{La codifica di Huffman}
I coefficienti AC sono codificati senza perdite secondo il \textit{run-length encoding} (RLE) e la codifica di Huffman. Vengono formate tabelle (per Y, Cb e Cr) su base statistica. I coefficienti DC codificano le differenze tra blocchi dei macroblocchi.

\parencite{jurgem_1997}

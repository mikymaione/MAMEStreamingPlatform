...
Nel main del programma è implementata la funzione \verb|on_accept| che avvia l'emulazione della macchina arcade selezionata, come mostrato in Lis. \ref{lst:sdlmain_callback}.
...
La piattaforma offre all'utente la possibilità di poter giocare sia tramite tastiera che tramite joystick e consentire il multiplayer sullo stesso dispositivo, anche sui dispositivi mobili tramite periferiche Bluetooth. Per far ciò sono state utilizzate due librerie JavaScript open source per la gestione dell'input:
...
Come mostrato nel Lis. \ref{lst:InitInput} presente nel file \verb|MAME.js| la funzione \verb|InitKeyboard| si occupa di inizializzare la libreria Keypress e la funzione \verb|InitGamepad| della libreria GameController. I tasti mappati sono le quattro direzioni, gli otto tasti azione e i tasti funzione start, coin, pausa. Nel listato è mostrata la mappatura del tasto pausa per la tastiera e del tasto R2 per il joystick. In ambedue i casi deve essere gestita la pressione e il rilascio.

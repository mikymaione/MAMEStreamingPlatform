...
Il cloud gaming effettua lo streaming in tempo reale ed è quindi essenziale garantire un livello di qualità del flusso audiovisivo percepito. Lo streaming video a bit-rate e frequenza dei fotogrammi bassi è possibile preservando la qualità percepita ed è particolarmente adatto per l'utilizzo in reti mobili\footnote{Con l'utilizzo di tariffe flat per reti mobili. Solitamente sono abbonamenti solo dati per tablet e smartphone.} e WLAN \parencite{VideoAndMultimediaTransmissionsOverCellularNetworks}.



\subsection{Video}
Qualsiasi elaborazione applicata ad un'immagine può causare perdita di qualità. La qualità dell'immagine si riferisce al modo in cui viene riprodotta la scena ripresa e può essere descritta, ad esempio, dal degrado della nitidezza, della gamma dinamica, della fusione dei colori, del contrasto, dell'effetto di blocco o della sfocatura. I metodi di valutazione della qualità dell'immagine possono essere suddivisi in metodi oggettivi e soggettivi. I metodi oggettivi si basano su confronti utilizzando criteri numerici espliciti e sono possibili diversi riferimenti come la verità fondamentale o le conoscenze pregresse espresse in termini di parametri e test statistici. L'errore quadratico medio (MSE), il \textit{peak signal-to-noise ratio} (PSNR) e la \textit{structural similarity index measure} (SSIM) sono esempi di misure di qualità utilizzate nell'analisi delle immagini.



\subsubsection{Peak signal-to-noise ratio}
Il PSNR è comunemente usato come misura della qualità della ricostruzione nella compressione e nella riduzione del rumore dell'immagine \parencite{relationship_PSNR_and_SSI}.
...
\begin{equation} \label{eq:PSNR}
	PSNR(f,g)=10 \log_{10}  \frac{255^2}{MSE(f,g)}	
\end{equation}


\subsubsection{Structural Similarity Index Measure}
SSIM è considerato in correlazione con la percezione della qualità del sistema visivo umano. Invece di utilizzare i tradizionali metodi di somma degli errori, SSIM è progettato modellando qualsiasi distorsione dell'immagine come una combinazione di tre fattori che sono la perdita di correlazione, la distorsione della luminanza e la distorsione del contrasto \parencite{relationship_PSNR_and_SSI}.

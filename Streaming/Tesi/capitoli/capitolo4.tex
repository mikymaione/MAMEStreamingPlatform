...
In questo progetto è stata usata un'implementazione per scopi accademici di PEAQ a cura di Giuseppe Gottardi\footnote{Il repository del progetto Peaq è disponibile qui: sourceforge.net/projects/peaqb.}.

\begin{enumerate}
    \item molto fastidioso;
    \item fastidioso;
    \item leggermente fastidioso;
    \item percettibile ma non fastidioso;
    \item impercettibile.
\end{enumerate}

\subsubsection{Risultati ottenuti}
Tramite lo script bash mostrato in Lis. \ref{lst:bashPEAQ} è stato calcolato per la codifica \textit{MPEG-1 Audio Layer II} (MP2) a 64 kbps un grado PEAQ di 3 (leggermente fastidioso).
...
peaqb -r Castelvania_PCM.wav -t Castelvania_MP2.wav
\end{lstlisting}

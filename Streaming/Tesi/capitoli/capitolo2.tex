...
Per quanto riguarda la libreria SDL viene creato il contesto di rendering \verb|SDL_Renderer| per la finestra tramite la funzione \verb|SDL_CreateRenderer|. La fase di disegno sul framebuffer è fatta tramite le funzioni \verb|SDL_SetRenderDrawColor| per settare il colore, \verb|SDL_RenderDrawLine| e \verb|SDL_RenderFillRect| per disegnare rispettivamente le primitive \verb|LINE| e \verb|QUAD|. Al termine della fase di disegno tramite la funzione \verb|SDL_RenderPresent| viene mostrato il framebuffer appena renderizzato sulla finestra.

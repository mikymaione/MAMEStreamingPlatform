...
Come detto precedentemente le due tecnologie alla base del cloud gaming sono il cloud computing e lo streaming. Il cloud computing è un paradigma in cui un provider offre a pagamento risorse fisiche e software accessibili in remoto. I modelli di servizio\footnote{La sigla aaS sta per "as a Service".} offerti sono: infrastruttura (IaaS), piattaforma (PaaS) e software (SaaS). Il cloud gaming è l'unione del modello PaaS e del modello SaaS, perché i videogiochi sono offerti come un servizio dal punto di vista del giocatore, ma da quello dello sviluppatore il modello offerto è il PaaS, secondo \citeauthor{Cloud_for_Gaming} \cite{Cloud_for_Gaming} il paradigma implementato dal cloud gaming è il gioco come servizio (GaaS). GaaS si divide in differenti instanze, come mostrato in Fig. \ref{fig:GaaS}, che sono:
...
\begin{itemize}
    \item Rendering remoto (RR-GaaS): il gioco viene eseguito, codificato ed inviato all'utente sotto forma di filmato;
    \item Rendering locale (LR-GaaS): il gioco viene eseguito, codificato ed inviato all'utente sotto forma di istruzioni di rendering. Il client di inviare le istruzioni alla scheda grafica dell'utente;
    \item Allocazione delle risorse cognitive (CRA-GaaS): il client riceve parti dell'eseguibile del gioco che vengono eseguite sul dispositivo dell'utente.
\end{itemize}

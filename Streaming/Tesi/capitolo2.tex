...
\begin{itemize}
	\item Core in cui ci sono i sotto-progetti indipendenti dal sistema e dal device emulato, il programma main a linea di comando (progetto main), il front-end grafico, il motore di emulazione (progetto emu), alcune librerie comuni (lib) ed i sorgenti delle librerie esterne (3rdparty);
	...
	\item Devices che contiene per ogni device emulato le classi che gestiscono le informazioni della macchina ed emulano cpu, bus, video, audio e i dischi (progetto formats).
\end{itemize} 

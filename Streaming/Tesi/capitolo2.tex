...
\begin{itemize}
	...
	\item OSD contenente le funzionalità sia dipendenti dal sistema operativo come Windows, UWP\footnote{UWP - Universal Windows Platform è architettura applicativa della Microsoft per sviluppare applicazioni eseguibili su Windows 10, Windows 10 Mobile, Xbox One e Hololens.} ed SDLMAME\footnote{SDLMAME è un port di MAME, ora integrato nel progetto MAME, che utilizza la libreria SDL in grado di funzionare su Windows, Linux, macOS ed altri sistemi operativi supportati dalla libreria SDL.}, sia i moduli di input, audio e video dipendenti da altre librerie come OpenGL, DirectX, SDL, CoreAudio, XAudio, XInput, ecc\dots;
	\item La macro area Devices contiene per ogni device emulato le classi che gestiscono cpu, bus, video e audio.
\end{itemize} 
